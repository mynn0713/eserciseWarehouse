...
public class MyDispatcherServlet extends HttpServlet {
    ...
    //private List<String> powerUsers = new ArrayList<>();
    ...
    /*private void doUserSecurty() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            if(aClass.isAnnotationPresent(Security.class)) {
                ...
                powerUsers.addAll(Arrays.asList(annotation.value())); // 等同于/demo
            }
        }
...
//        method.invoke() //
        ...
        boolean canSecurity = false;
        //判断@Security权限
        boolean hasSecurityController = handler.getController().getClass().isAnnotationPresent(Security.class);
        if(hasSecurityController){
            Security controllerSecurity = handler.getController().getClass().getAnnotation(Security.class);
            String[] users = controllerSecurity.value();
            if(Arrays.asList(users).contains(parameterMap.get("userName"))){
                canSecurity=true;
            }
            if(!canSecurity){
                resp.getWriter().write("无权限用户Controller");
                return;
            }
        }
        boolean hasSecurityMethod = handler.getMethod().isAnnotationPresent(Security.class);
        if(hasSecurityMethod){
            Security methodSecurity = handler.getMethod().getAnnotation(Security.class);
            String[] users = methodSecurity.value();
            if(Arrays.asList(users).indexOf(parameterMap.get("userName")[0])!=-1){
                canSecurity=true;
            }
            if(!canSecurity){
                resp.setContentType("text/html;charset=utf-8");
                resp.getWriter().write("无权限用户method");
                return;
            }
        }
